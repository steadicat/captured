...
import {Button, Input, MainButton} from '../ui/core';
...
const StatusSelector = component(
  'StatusSelector',
  ({get, actions, status, ...props}) => (
    <Button
      fontWeight="bold"
      padding={6}
    ...
    return (
      <Block tag="form" onSubmit={this.onSubmit} {...props}>
        <Input
          ...
          marginRight={6}
        />
        <AdminButton tag="button" type="submit">
          Mark Shipped
        </AdminButton>
      </Block>
    );
...
function formatDate(date) {
  ...
  return `${d.getMonth() + 1}/${d.getDate()} ${d.getHours()}:${pad(
    d.getMinutes()
  )}`;
}
...
const Order = component(
  ...
  ({
    ...
    <Block marginBottom={12}>
      <Column width="20%">
        <LightCondensedText fontSize={12}>
          {formatDate(created)}
        </LightCondensedText>
      </Column>
      <Column width="40%">
        ...
        {Object.entries(metadata)
          .filter(key => key !== 'tracking_number')
          .map(([key, value]) => (
      ...
      <Column width="40%">
        {status === 'created' && (
          <AdminButton onClick={() => actions.chargeOrder(id, customer.id)}>
            Charge
          </AdminButton>
        )}
        {status === 'charging' && <AdminButton>...</AdminButton>}
        {status === 'failed' && (
          <Block color="#e52" fontWeight="BOLD">
            DECLINED
          </Block>
        )}
        {status === 'canceled' && (
        ...
        {status === 'paid' && (
          <Tracking onShipped={tracking => actions.shipOrder(id, tracking)} />
        )}
        {status === 'fulfilled' && (
          <Block>Shipped: {metadata.tracking_number}</Block>
        )}
      </Column>
    </Block>
  )
...
export class OrdersClass extends React.Component {
  ...
  render() {
    ...
    return (
      <DefaultFont>
        <Block padding={24}>
          <PageHeading textAlign="center" marginTop={24} marginBottom={24}>
            Orders
          </PageHeading>
          ...
          {!get('ordersLoading') && !get('orders').size && (
...
export const Orders = connect(OrdersClass);
