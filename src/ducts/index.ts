...
function getKey(store, key, defaultValue) {
  ...
  const val = key
    .split('.')
    .reduce((data, bit) => getSingleKey(data, bit), store);
  return val !== undefined && val !== null ? val : defaultValue;
}
    ...
    boundActions[k] = function(...args) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        if (newStore && call.callees.length) {
          console.warn(
            `Action ${call.caller} called ${call.callees.join(
              ', '
            )} synchronously, which modified the store, then modified the store itself, potentially overriding the changes. Try to split up actions that call other actions and actions that modify the store.`
          );
        }
      }
